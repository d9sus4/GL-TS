...
def main():
	...
	for i, segment in enumerate(segments):
		...
		if i + 1 <= int(RATIO * ds_size):
			train_x.append(obs_padded)
			train_y.append(label)
		else:
			test_x.append(obs_padded)
			test_y.append(label)
	...
	x = np.expand_dims(train_x, axis=1)  # Axis 1 is after the first dimension
	print(x.shape)
	model.fit(x)
	features = model(x)
...
def main_backup():
	...
	timer = timerClass()
	layer1 = torchSaab(kernel_size=(4,4), stride=(2,2), channelwise=False)
